...
async function getScienceFictionMovies(res) {
  ...
  for (const movie of data) {
    ...
    mongoose.connect(config.mongoDB, async (err, db) => {
      if (err) {
        ...
        const database = db.collection("science_fiction_movies");
        try {
...
route.post("/train_db", async (req, res) => {
  ...
  const POPULAR_MOVIES = `${DOMAIN_URL}/3/movie/popular?api_key=${API_KEY}&page=${page}&with_genres=878`;
  const popularMovies = await axios.get(POPULAR_MOVIES);

  await getScienceFictionMovies(popularMovies);
